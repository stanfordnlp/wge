...
window.addEventListener("load", function () {
  ...
  warningDiv.innerHTML = "<p><strong>Important: Please use <span class=red>Google Chrome on desktop</span>. The task may not work correctly on other browsers!</strong><br>Please report any problem to the requester.</p>";
...
core.endEpisode = function(reward, time_proportional) {
  ...
  core.updateDisplay(WOB_RAW_REWARD_GLOBAL > 0 ? Math.floor(WOB_RAW_REWARD_GLOBAL * 10) / 10 : 0);
  core.clearTimer();
  core.startEpisode();
}
...
core.DISPLAY_HTML = `
  <div class="info">
    ...
    <span id='reward-total'>0</span> / <span class='episodes-needed'>2</span>
  </div>
  <div class="info">
    <label>Time left for current task:</label>
    <span id='timer-countdown'>-</span>
  </div>
  <div class="info">
    <label>Result of last task:</label>
...
core.INSTRUCTION_HTML = `
  <p><strong>Important: Please use <span class=red>Google Chrome on desktop</span>. The task may not work correctly on other browsers!</strong></p>
  <h1>Instructions</h1>
  <p>In each task, please demonstrate how to fill the search form to search for flights based on the given criteria.</p>
  <p>You can submit the HIT after successfully completing <span class='episodes-needed'>2</span> tasks.</p>
  <h1>Details</h1>
  <p><strong>Required fields:</strong> You must fill in all city and date fields. If the flight is one-way, be sure to select the "One-way" option to disable the return date field.</p>
  <p>If all required fields are filled but some fields are wrong, you will get partial credit.</p>
  <p><strong>Cities:</strong> Please use autocomplete to select the cities. For ambiguous airports, use the following:</p>
  <p>&nbsp;&nbsp;&nbsp;&nbsp;Portland &rarr; Portland, OR (not MD)</p>
  <p>&nbsp;&nbsp;&nbsp;&nbsp;Dallas &rarr; Dallas, TX (not Ft. Worth)</p>
  <p>&nbsp;&nbsp;&nbsp;&nbsp;New York &rarr; the "All Airports" option</p>
  <p>&nbsp;&nbsp;&nbsp;&nbsp;Baltimore &rarr; Baltimore, MD (not Washington DC)</p>
  <p><strong>Ticket Type:</strong> "Return flight" means round-trip flight (i.e., "One-way" option left unchecked)</p>
  <p><strong>Dates:</strong> Use the default month and year (which should be March 2017).</p>
  <p><strong>Seat type:</strong> Expand the "More search options" section to select the seat type.</p>
  <p><strong>Problems:</strong> If the interface gets stuck, try pressing the Reset button. If the problem persists, please contact the requester.</p>
`;
...
core.createDisplay = function(){
  ...
  if(display === null) {
    ...
    instructionDiv.setAttribute('id','instructions');
    instructionDiv.innerHTML = core.INSTRUCTION_HTML;
    document.body.appendChild(instructionDiv);
    // On-screen text
    document.getElementById('query-pretty').textContent = 
      '(Flight criteria will appear here)';
    document.getElementById('reward-reason').textContent =
      '(Feedback will appear here)';
    core.EPISODE_MAX_TIME = 120000; // OVERRIDE!
    // Reward display
    var rewardDiv = document.createElement('div');
    rewardDiv.setAttribute('id','reward-display');
    rewardDiv.innerHTML = core.DISPLAY_HTML;
    document.getElementById('query-wrap').insertBefore(
        rewardDiv, document.getElementById('query-pretty'));
    document.getElementById('resetButton').onclick = function (e) {
      core.endEpisode(-1, false, 'RESET');
    };
    // Turk form
...
core.updateDisplay = function(reward){
  ...
  if(core.wob_latest !== '-'){
    var latestText = (core.wob_latest == 1 ? 'success' : core.wob_latest > 0 ? 'partial' : 'failure');
    var latestColor = (core.wob_latest == 1 ? 'green' : core.wob_latest > 0 ? 'orange' : 'red');
...
recorder.endRecording = function () {
  ...
  if (core.wob_scores >= core.EPISODES_NEEDED) {
